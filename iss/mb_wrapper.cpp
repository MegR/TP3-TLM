...
void MBWrapper::exec_data_request(enum iss_t::DataAccessType mem_type,
	...
	case iss_t::READ_BYTE: {
		uint32_t offset = mem_addr % 4;
		socket.read(mem_addr - offset, localbuf);
		localbuf = (localbuf >> ((3 - offset) * 8)) & 0xFF;
	...
	while (true) {
		if (m_iss.isBusy())
			...
			if (ins_asked) {
				...
				m_iss.setInstruction(0, 
						     uint32_machine_to_be(
							     localbuf));
			}
